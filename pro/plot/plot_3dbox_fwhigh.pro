;  $Id: //depot/idl/IDL_70/idldir/lib/plot_3dbox.pro#1 $
;
...
PRO Plot_3dbox_fwhigh, X, Y, Z, COLOR_walls=COLOR, BACKGROUND=BACKGROUND,        $  
                       ...
                       color_3d=color_3d, symsize=symsize, $
                       overplot=overplot, nodata_3d=nodata_3d, onlydata_3d=onlydata_3d, $
                       _EXTRA=e       
...
; And now plot the data along the walls of the box if requested
  if not keyword_set(onlydata_3d) then begin
     if (KeyWord_Set(XY_PLANE)) then begin 
        here=where(x gt !x.crange[0] and x lt !x.crange[1] and $
                   y gt !y.crange[0] and y lt !y.crange[1] , count)
        if count ge 2 then begin
           Plots, X[here], Y[here], Replicate(!Z.Crange[0], n_elements(here)), /T3D, COLOR=COLOR, $
                  LINESTYLE=XYSTYLE, psym=psym, symsize=symsize, $
               _EXTRA=e
        endif
     endif
     if(KeyWord_Set(YZ_PLANE))then begin
        here=where(y gt !y.crange[0] and y lt !y.crange[1] and $
                   z gt !z.crange[0] and z lt !z.crange[1] , count)
        if count ge 2 then begin
        Plots, Replicate(!X.Crange[1], n_elements(here)), Y[here], Z[here], /T3D, COLOR=COLOR, $
               LINESTYLE=YZSTYLE, psym=psym, symsize=symsize, $
               _EXTRA=e
        endif
     endif
     if(KeyWord_Set(XZ_PLANE))then begin
        here=where(x gt !x.crange[0] and x lt !x.crange[1] and $
                   z gt !z.crange[0] and z lt !z.crange[1] , count)
        if count ge 2 then begin
        Plots, X[here], Replicate(!Y.Crange[1], n_elements(here)), Z[here], /T3D, COLOR=COLOR, $
               LINESTYLE=XZSTYLE, psym=psym, symsize=symsize, $
               _EXTRA=e
        endif
     endif
  endif
...
; now plot the data     
  if not keyword_set(nodata_3d) then begin
     here=where(x gt !x.crange[0] and x lt !x.crange[1] and $
                y gt !y.crange[0] and y lt !y.crange[1] and $
                z gt !z.crange[0] and z lt !z.crange[1] , count)
     
     if count ge 2 then begin
        Plots, X[here], Y[here], Z[here], /T3D , PSYM=PSYM, COLOR=color_3d ,$
